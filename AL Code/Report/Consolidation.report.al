report 60007 "Import Consolidation Non GAAP"
{
    Caption = 'Statistical Consolidation Report';
    ProcessingOnly = true;
    ...
    dataset
    {
        dataitem("Business Unit"; "Business Unit")
        {
            ...
            dataitem("Statistical Account"; "Statistical Account")
            {
                ...
                dataitem("Statistical Ledger Entry"; "Statistical Ledger Entry")
                {
                    DataItemLink = "Statistical Account No." = field("No.");
                    DataItemTableView = SORTING("Statistical Account No.", "Posting Date");
                    dataitem(StatsDimensionSetEntry; "Dimension Set Entry")
                    {
                        DataItemLink = "Dimension Set ID" = FIELD("Dimension Set ID");
                        DataItemTableView = SORTING("Dimension Set ID", "Dimension Code");
                    ...
                    begin
                        InitGLEntry(NewGLEntry);
                        GLEntryNo := BusUnitConsolidate.InsertGLEntry(NewGLEntry);
                    end;
                    ...
                    begin
                        ...
                        if GetRangeMin("Posting Date") = NormalDate(GetRangeMin("Posting Date")) then
                            CheckStatsClosingPostings("Statistical Account"."No.", GetRangeMin("Posting Date"), GetRangeMax("Posting Date"));
                    end;
                }

                trigger OnAfterGetRecord()
                var
                    MapiingAccount: Record "G/L Account";
                begin
                    Window.Update(2, "No.");
                    Window.Update(3, '');
    ...
    begin
        DimSelectionBuf.CompareDimText(
          3, REPORT::"Import Consolidation Non GAAP", '', ColumnDim, Text020);
        GLSetup.GetRecordOnce();
    ...
    begin
        ...
        AccountingPeriod.SetRange("Starting Date", StartDate + 1, EndDate);
        if AccountingPeriod.Find('-') then begin
            StatisticalEntry.ChangeCompany("Business Unit"."Company Name");
            StatisticalEntry.SetRange("Statistical Account No.", GLAccNo);
            repeat
                StatisticalEntry.SetRange("Posting Date", ClosingDate(AccountingPeriod."Starting Date" - 1));
                if not StatisticalEntry.IsEmpty() then
                    Error(
                      Text022,
                      StatisticalEntry.TableCaption(),
                      StatisticalEntry.FieldCaption("Posting Date"),
                      StatisticalEntry.GetFilter("Posting Date"),
                      StatisticalEntry.FieldCaption("Statistical Account No."),
                      GLAccNo);
            until AccountingPeriod.Next() = 0;
        end;
    end;
    ...
    procedure InitGLEntry(Var NewGLEntry: Record "G/L Entry")
    begin
        NewGLEntry."Debit Amount" := 0;
        NewGLEntry."Credit Amount" := 0;
        NewGLEntry."G/L Account No." := GetAccountNo;
        NewGLEntry."Posting Date" := "Statistical Ledger Entry"."Posting Date";
